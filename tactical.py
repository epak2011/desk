...
def tactical_action(bias, setup_score, atr_ok):
    if not atr_ok: return "avoid"
    if bias != "bullish": return "avoid"
    if setup_score >= 9: return "enter_now"
    return "watch"
...
def compute(ticker_hist, bench_hist, atr_threshold=0.015):
    ...
    action = tactical_action(bias, setup, atr_ok)
